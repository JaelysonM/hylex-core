...
public class SkinApplier {
  ...
  public SkinApplier(Bungee plugin) {
    this.plugin = plugin;
  }
  ...
  public void applySkin(final ProxiedPlayer p, final String nick, InitialHandler handler) throws Exception {
    ...
    if (profile == null) {
      try {
        ...
        profile =
          (net.md_5.bungee.connection.LoginResult) ReflectionUtil.invokeConstructor(LoginResult, new Class<?>[] {String.class, Property[].class}, null, new Property[] {textures});
      }
    ...
    if (Bungee.getInstance().isMultiBungee()) {
      if (p != null)
        sendUpdateRequest(p, textures);
    } else {
      if (p != null)
  ...
  private void sendUpdateRequest(ProxiedPlayer p, Property textures) {
    ...
    if (Config.DEBUG)
      System.out.println("[hylex-core Fork SkinsRestorer] Sending skin update request for " + p.getName());
    ...
    try {
      ...
      p.getServer().sendData("hylex-core", b.toByteArray());
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
